...
if [ "$1" = "" ]; then
  (exec  nohup $JAVA_HOME/bin/java -server -Xms96m -Xmx128m -cp $TC/bin/bootstrap.jar -Djava.awt.fonts=$JAVA_HOME/jre/lib/fonts -Djava2d.font.usePlatformFont=false -Djava.awt.headless=true -Djavax.xml.parsers.DocumentBuilderFactory=org.apache.xerces.jaxp.DocumentBuilderFactoryImpl -Djavax.xml.parsers.SAXParserFactory=org.apache.xerces.jaxp.SAXParserFactoryImpl -Dfedora.home=$FEDORA_HOME -Dclasspath=$TC/bin/bootstrap.jar -Djava.endorsed.dirs=$TC/bin -Dcatalina.base=$TC -Dcatalina.home=$TC -Djava.io.tmpdir=$TC/temp org.apache.catalina.startup.Bootstrap start &)
else
