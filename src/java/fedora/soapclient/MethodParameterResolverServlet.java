 ...
 * into an appropriate dissemination request and then executes the dissemination
 * request.
 * </p>
 * 
 * @author rlw@virginia.edu
 * @version $Id: MethodParameterResolverServlet.java,v 1.11 2005/04/21 13:28:49
 *          rlw Exp $
 */
public class MethodParameterResolverServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		while (parms.hasMoreElements()) {
			...
			if (name.equals("PID")) {
				PID = URLDecoder.decode((String) request.getParameter(name),
						"UTF-8");
			} else if (name.equals("bDefPID")) {
				bDefPID = URLDecoder.decode(
						(String) request.getParameter(name), "UTF-8");
			} else if (name.equals("methodName")) {
				methodName = URLDecoder.decode((String) request
				...
				h_methodParms.put(URLDecoder.decode(name, "UTF-8"), URLDecoder
						.decode((String) request.getParameter(name), "UTF-8"));
			}
		}
