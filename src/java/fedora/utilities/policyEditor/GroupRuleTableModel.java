...
public class GroupRuleTableModel extends AbstractTableModel {
	...
	final private static String columnNames[] = { "Permit/Deny",
			"Name of User Group", "Description of User Group" };
	...
	public GroupRuleTableModel(int showTemplatesOrRules,
			boolean showPermitDenyColumn, int permitDenyOrBoth) {
		super();
		columnCount = showPermitDenyColumn ? 3 : 2;
		this.permitDenyOrBoth = permitDenyOrBoth;
	...
	public GroupRuleInfo getRow(int row) {
		...
		if (templatesOrRules == TEMPLATES) {
			if (permitDenyOrBoth == PERMIT) {
				rowinfo = getRowWithParameters(row,
			...
			if (permitDenyOrBoth == PERMIT) {
				rowinfo = (GroupRuleInfo) GroupRuleInfo.permitRules
						.elementAt(row);
			} else if (permitDenyOrBoth == DENY) {
				rowinfo = (GroupRuleInfo) GroupRuleInfo.denyRules
						.elementAt(row);
			} else // BOTH
			{
				if (row < GroupRuleInfo.permitRules.size()) {
					...
					rowinfo = (GroupRuleInfo) GroupRuleInfo.denyRules
							.elementAt(row - GroupRuleInfo.permitRules.size());
				}
			}
			return (rowinfo);
		}
	...
	public int getRowNum(int row) {
		...
		if (templatesOrRules == TEMPLATES) {
			if (permitDenyOrBoth == PERMIT) {
				num = getRowNumWithParameters(row,
						GroupRuleInfo.permitTemplates);
			} else // if (permitDenyOrBoth == DENY)
			{
	...
	public int getRowCount() {
		if (templatesOrRules == TEMPLATES) {
			...
			default:
			case BOTH:
				return (GroupRuleInfo.permitRules.size() + GroupRuleInfo.denyRules
						.size());
			}
		}
	}
