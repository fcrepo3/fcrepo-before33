 ...
 * to a Log4J <code>Logger</code>.
 *
 * This class is useful if your project wants consistent logging configuration
 * based on Log4J, but must work with a library that uses the Java Logging API.
 * By using this filter's convenience method, <code>apply</code>, messages 
 * that would otherwise go to Java Logging handler(s) are instead sent to 
 * the configured Log4J appender(s).
 *
 * Example use:
 * <pre>
...
public class Log4JRedirectFilter
    ...
    /**
     * Convenience method for applying a <code>Log4JRedirectFilter</code> 
     * to a Java Logger, by name.
     *
     * Log records destined for the given Java logger will be redirected
     * to the Log4J logger of the same name.
     */
    ...
    /**
     * Always returns false, but before doing so, sends the given
     * record to the Log4J Logger.
     */
    ...
    private LoggingEvent getLoggingEvent(LogRecord record) {
        ...
        return new CustomLoggingEvent(FQCN, _log4jLogger, log4jLevel,
                record);
    }
    ...
    /**
     * This subclass is necessary because we need to force the source 
    ...
    class CustomLoggingEvent extends LoggingEvent {
        ...
        public CustomLoggingEvent(String callerFQCN, Logger logger,
                Level level, LogRecord record) {
            super(callerFQCN, logger, level, record.getMessage(), 
                    record.getThrown());
            _record = record;
        }
        ...
        public synchronized LocationInfo getLocationInformation() {
            if (_locationInfo == null) {
                ...
                _locationInfo = new CustomLocationInfo(stackInfo, 
