...
//      Lay out the buttons from left to right.
    ...
    public void setContent(InputStream data) throws IOException 
    {
        ...
        if (m_origContent == null)  m_origContent = (RDFDataModel) m_dataModel.clone();
    }
    ...
    {
        return (!m_origContent.serializeAsString().equals(m_dataModel.serializeAsString()));
    }
    ...
    class RDFDataModel extends AbstractTableModel  
    {
        ...
        private RDFDataModel() {}  // only for use by clone
        ...
        public RDFDataModel(InputStream data)
        {
            ...
            try
            {
                ...
                for (int i = 0; iter.hasNext(); i++)
                {
                    ...
                    {
                        objLiteral = object.toString();
                        URI typeURI = ((Literal)object).getDatatypeURI();
                        literalType = (typeURI == null) ? null : typeURI.toString();
                     }
                    else
                    {
                        objURI = object.toString();
                    }                        
                    entries.add(new RelationshipTuple(triple.getSubject().toString(),
                                                      triple.getPredicate().toString(), 
                                                      objURI, objLiteral, literalType) );
        ...
        public void addRow(String subjectURI, String predicate, String objectURI, 
                           String literalValue, String literalType)
        {
            entries.add(new RelationshipTuple(subjectURI, predicate, objectURI, literalValue, literalType));
            dataChanged();
        }
        
        public void replaceRow(int selectedRow, String subjectURI, String predicate, String objectURI, String literalValue, String literalType)
        {
            entries.set(selectedRow, new RelationshipTuple(subjectURI, predicate, objectURI, literalValue, literalType));
            dataChanged();            
        }   

        public InputStream serializeAsStream()
        {
            TupleArrayTripleIterator iter = new TupleArrayTripleIterator(entries);
        ...
        public Object getValueAt( int iRowIndex, int iColumnIndex )
        {
            ...
            switch (iColumnIndex)
            {
                case 0: return tuple.subjectURI;
                case 1: 
                    if (tuple.predicate.startsWith("info:fedora/fedora-system:def/relations-external#"))
                    {
                ...
                case 2: 
                    if (tuple.objectURI != null) return tuple.objectURI;
                    if (tuple.objectLiteral != null)  return "\"" + tuple.objectLiteral + "\"";
                    return("");
            }
            return("");
        }
    ...
    public void actionPerformed(ActionEvent e)
    {
        if (e.getActionCommand().equals("Add..."))
        {
            ...
            if (!dialog.isCancelled())
            {
                ...
                String literalValue = dialog.getLiteralVal();
                String literalType = dialog.getLiteralType(); 
                m_dataModel.addRow(subject, predicate, objectURI, literalValue, literalType); 
            }
            dialog = null;
        }
        else if (e.getActionCommand().equals("Edit..."))
        {
            ...
            {
                TripleEditDialog dialog = new TripleEditDialog(m_dataModel.entries.get(m_editor.getSelectedRow()), m_map);
                dialog.setVisible(true);
                if (!dialog.isCancelled())
                {
                    String subject = dialog.getSubject();
                    ...
                    String literalValue = dialog.getLiteralVal();
                    String literalType = dialog.getLiteralType(); 
                    m_dataModel.replaceRow(m_editor.getSelectedRow(), subject, predicate, objectURI, literalValue, literalType); 
                }
                dialog = null;
            }
        }
    ...
    class TripleEditDialog extends JDialog implements ActionListener, DocumentListener
    {
        ...
        private JTextField m_literalVal;
        ...
        public TripleEditDialog(RelationshipTuple tuple, HashMap<String,String> map)
        {
            ...
            mainPanel.add(m_subject = new JTextField(tuple != null ? tuple.subjectURI : PID.toURI(pid)));
            ...
            String rels[] = {"", "rel:hasBDef", "rel:isContractor", "rel:hasFormalContentModel", 
                            "rel:isMemberOf"};
            ...
            mainPanel.add(lab3 = new JLabel("Object URI:", SwingConstants.RIGHT));
            mainPanel.add(m_objectURI = new JTextField(""));
            m_objectURI.getDocument().addDocumentListener(this);
            
            mainPanel.add(new JLabel("    or    ", SwingConstants.RIGHT));
            mainPanel.add(new JLabel(""));
            
            mainPanel.add(lab4 = new JLabel("Literal Value:", SwingConstants.RIGHT));
            mainPanel.add(m_literalVal = new JTextField(""));
            m_literalVal.getDocument().addDocumentListener(this);

            mainPanel.add(lab5 = new JLabel("      Type:", SwingConstants.RIGHT));
            String types[] = {"<untyped>", "string", "normalizedString", "boolean", "decimal", 
                              "long", "int", "short", "byte", "float", "double",
                              "duration", "dateTime", "time", "date", "hexBinary", "base64Binary",
                              "anyURI", "QName"};
            ...
            if (tuple != null)
            {
                m_subject.setText(tuple.subjectURI);
                m_predicate.setSelectedItem("rel:" + tuple.getRelationship());
                m_objectURI.setText(tuple.objectURI == null ? "" : tuple.objectURI);
                m_literalVal.setText(tuple.objectLiteral == null ? "" :tuple.objectLiteral);
                m_literalType.setSelectedItem(tuple.literalType == null ? "<untyped>" : tuple.literalType);
                if (m_objectURI != null)
                {
                    m_objectURI.setEnabled(true);
                    m_literalVal.setEnabled(false);
                    m_literalType.setEnabled(false);
                }
                else if (m_literalVal != null)
                {
                    m_objectURI.setEnabled(false);
                    m_literalVal.setEnabled(true);
        ...
        public void actionPerformed(ActionEvent arg0)
        {
            if (arg0.getActionCommand().equals("OK"))
            {
                ...
                try
                {
                    ...
                    TupleArrayTripleIterator.makePredicateResourceFromRel(getPredicate(), m_map);
                    msg = "object";
                    TupleArrayTripleIterator.makeObjectFromURIandLiteral(getObjectURI(), getLiteralVal(), getLiteralType());
                }
        ...
        public void updateFields()
        {
            ...
            else if (m_literalVal.getText().length() > 0)
            {
                m_objectURI.setEnabled(false);
            }
            else
        ...
        public String getLiteralVal()
        {
            ...
            if (m_literalVal.getText().length() == 0) 
            {
                return null;
            }
            return m_literalVal.getText();            
        }
