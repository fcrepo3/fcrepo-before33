...
public class MethodPropertiesDialog
    ...
    private final HashMap<String, String> parmTypeTbl =
            new HashMap<String, String>();

    private final HashMap<String, String> parmTypeToDisplayTbl =
            new HashMap<String, String>();
    ...
    private final HashMap<String, String> passByTbl =
            new HashMap<String, String>();

    private final HashMap<String, String> passByToDisplayTbl =
            new HashMap<String, String>();
    ...
    private final HashMap<String, String> parmReqTbl =
            new HashMap<String, String>();

    private final HashMap<String, String> parmReqToDisplayTbl =
            new HashMap<String, String>();
    ...
    public MethodPropertiesDialog(MethodsPane parent,
                                  String methodName,
        ...
        this.parent = parent;
        builderClassName = parent.parent.getClass().getName();
        ...
        if (builderClassName.equalsIgnoreCase(ServiceDeploymentBuilder.class
                .getName())) {
            // this.baseURL = parent.getBaseURL();
            if (parent.hasBaseURL()) {
                baseURL =
                        parent.getBaseURL().endsWith("/") ? parent.getBaseURL()
                                : parent.getBaseURL() + "/";
            }
    ...
    private JPanel setBindingPanel() {
        ...
        rb_httpDS =
                new JRadioButton("Fedora LOCAL HTTP Resolver: (enter datastream parm name)",
                                 false);
        rb_httpDS.setActionCommand("http");
        URL_textDS = new JTextField(30);
        URL_textDS
                .setToolTipText("Enter the datastream parameter name in parentheses."
                        + "  See Help button for more details on syntax.");

        // http - full URL
        rb_httpFull =
                new JRadioButton("Multi-Server Service: (enter full URL for method)",
                                 false);
        rb_httpFull.setActionCommand("httpFull");
        URL_textFull = new JTextField(30);
        URL_textFull
                .setToolTipText("Enter the full URL for the service method."
        ...
        URL_textRelative
                .setMinimumSize(new Dimension(200, (int) URL_textRelative
                        .getMinimumSize().height));
        URL_textRelative
                .setToolTipText("Enter relative URL for the service method"
                        + "  See Help button for more details on syntax.");
    ...
    private JPanel setParmsPanel() {
        ...
        if (builderClassName.equalsIgnoreCase(ServiceDeploymentBuilder.class
            ...
            parmTypes =
                    new String[] {(String) parmTypeToDisplayTbl
                            .get(MethodParm.USER_INPUT)};
        }
        ...
        String[] parmReq =
                (String[]) parmReqTbl.keySet().toArray(new String[0]);
        ...
        String[] parmPassBy =
                (String[]) passByTbl.keySet().toArray(new String[0]);
    ...
    private void addTableRow() {
        // Append a row
        ((DefaultTableModel) parmTable.getModel()).addRow(new Object[] {"", "",
                "", "", ""});
    }

    private void deleteTableRow() {
        ((DefaultTableModel) parmTable.getModel()).removeRow(parmTable
                .getSelectedRow());
    }
    ...
    private void helpTable() {
        ...
        helptxt
                .append("The Method Properties Pane is used to define method parameters"
                        + " for the selected method. Each parameter has the following properties:\n\n"
                        + " Name - the name of the parameter (required)\n\n"
                        + " Type - the type of the parameter. (required) For Service Definition objects,"
                        + " the only valid type is USER. For Service Deployment objects, valid types include"
                        + " USER (user parms), DATASTREAM (datastream parms), and DEFAULT (deployment default parms)\n\n"
    ...
    private void showURLHelp() {
        ...
        helptxt.setWrapStyleWord(true);
        helptxt
                .append("The Method Binding entry pane must specify an appropriate"
        ...
        helptxt
                .append("  A parameter placeholder is"
        ...
        helptxt
                .append("CASE 1: A relative URL representing a service method.\n\n"
        ...
        helptxt
                .append("/resize_image?image=(IMGDATASTREAM)&size=(USERSIZE)\n\n");
        helptxt
                .append("CASE 2: A fully qualified URL representing a service method.\n"
        ...
        helptxt
                .append("http://myserver.com/myservice/mymethod?myparm1=(parmname1)&myparm2=(parmname2)\n");
        helptxt.append("For example:\n");
        helptxt
                .append("http://localhost:8080/imgsizer/resize_image?image=(IMGDATASTREAM)&size=(USERSIZE)\n\n");
        helptxt
                .append("CASE 3:  No URL - just a 'DATASTREAM' replacement parameter.  This"
                        ...
                        + " that is bound to the Service method definition.  In this case, all"
                        ...
                        + " in response to running the method defined here.\n\n");
    ...
    private boolean parmsInURL(String http_URL, MethodParm[] parms) {
        for (MethodParm element : parms) {
            ...
            if (!foundInString(http_URL, pattern)) {
                if (element.parmName.equalsIgnoreCase("NULLBIND")) {
                    continue;
                }
                System.out.println("The parm named " + element.parmName
                        + " is not found in the HTTP method binding URL: "
                        + http_URL);
                return false;
            }
        }
        ...
        while (it.hasNext()) {
            ...
            if (m.find()) {
                ...
                while (st2.hasMoreTokens()) {
                    ...
                    parmsInURL.add(s);
                    // System.out.println("parm parsed out of HTTP method
                    // binding URL: " + s);
                    break;
                }
            }
    ...
    private boolean validMethodProperties(MethodProperties mp) {
        // System.out.println("MethodPropertiesDialog: validating method
        // properties...");
        return validBinding(mp) && validMethodParms(mp.methodParms)
                && validReturnTypes(mp.returnMIMETypes) ? true : false;
    }
    ...
    private boolean validBinding(MethodProperties mp) {
        ...
        if (builderClassName.equalsIgnoreCase(ServiceDeploymentBuilder.class
            ...
            if (mp.methodFullURL == null
                ...
                return false;
            } else if (!parmsFromURLMissing(mp.methodFullURL, mp.methodParms)) {
            ...
            /*
             * else if
             * (mp.protocolType.equalsIgnoreCase(mp.SOAP_MESSAGE_PROTOCOL)) {
             * assertMethodPropertiesMsg("Sorry, the SOAP bindings are not
             * supported yet." + " Please select HTTP binding."); return false; }
             */
        }
        return true;
    }
    ...
    private boolean validMethodParm(MethodParm parm) {
        if (parm.parmType == null || parm.parmType.trim().equalsIgnoreCase("")) {
            assertMethodPropertiesMsg("A value for 'Parm Type' must be selected for parm "
                    + parm.parmName);
            return false;
        } else if (parm.parmRequired == null
                || parm.parmRequired.trim().equalsIgnoreCase("")) {
            assertMethodPropertiesMsg("A value for 'Required?' must be selected for parm "
                    + parm.parmName);
            return false;
        } else if (parm.parmPassBy == null
                || parm.parmPassBy.trim().equalsIgnoreCase("")) {
    ...
    private String[] filterDSBindingKeys(MethodParm[] parms) {
        ...
        for (MethodParm element : parms) {
            if (element.parmType.equalsIgnoreCase(MethodParm.DATASTREAM_INPUT)) {
    ...
    private MethodParm[] unloadMethodParms() {
        ...
        for (int i = 0; i < rowcount; i++) {
            ...
            if (parmName != null && !parmName.equalsIgnoreCase("")
                ...
                parm.parmType =
                        (String) parmTypeTbl.get(parmTable.getValueAt(i, 1));
                parm.parmRequired =
                        (String) parmReqTbl.get(parmTable.getValueAt(i, 2));
                parm.parmPassBy =
                        (String) passByTbl.get(parmTable.getValueAt(i, 3));
                ...
                if ((values = (String) parmTable.getValueAt(i, 6)) != null) {
                    ...
                    StringTokenizer st =
                            new StringTokenizer(normalizedString, ",");
                    // System.out.println("count domain parms = " +
                    // st.countTokens());
                    while (st.hasMoreElements()) {
                        domainValues.add(((String) st.nextElement()).trim());
                    }
                    parm.parmDomainValues =
                            (String[]) domainValues.toArray(new String[0]);
                }
        ...
        for (int i = 0; i < parms.length; i++) {
            ...
            if (parms.length > freeRows) {
                ...
                for (int j = 0; j < newRows; j++) {
                    ((DefaultTableModel) parmTable.getModel())
                            .addRow(new Object[] {"", "", "", "", ""});
                }
            }
            ...
            ce.getTableCellEditorComponent(parmTable, parmTypeToDisplayTbl
                    .get(parms[i].parmType), true, i, 1);
            parmTable.setValueAt(ce.getCellEditorValue(), i, 1);

            ce = parmTable.getCellEditor(i, 2);
            ce.getTableCellEditorComponent(parmTable, parmReqToDisplayTbl
                    .get(parms[i].parmRequired), true, i, 2);
            parmTable.setValueAt(ce.getCellEditorValue(), i, 2);

            ce = parmTable.getCellEditor(i, 3);
            ce.getTableCellEditorComponent(parmTable, passByToDisplayTbl
                    .get(parms[i].parmPassBy), true, i, 3);
            ...
            // parms[i].parmDomainValues.length);
    ...
    private void renderBindingProperties(MethodProperties properties) {
        // if
        // (properties.protocolType.equalsIgnoreCase(Method.HTTP_MESSAGE_PROTOCOL))
        // {
        ...
        /*
         * else if
         * (properties.protocolType.equalsIgnoreCase(Method.SOAP_MESSAGE_PROTOCOL)) {
         * rb_httpDS.setSelected(false); rb_httpFull.setSelected(false);
         * rb_httpRelative.setSelected(false); URL_textDS.setText("");
         * URL_textFull.setText(""); URL_textRelative.setText("");
         * URL_textDS.setEnabled(false); URL_textFull.setEnabled(false);
         * URL_textRelative.setEnabled(false); rb_soap.setSelected(true); }
         */
    }
