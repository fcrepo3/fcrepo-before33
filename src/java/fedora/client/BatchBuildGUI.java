 ...
 * @version $Id$
 */
public class BatchBuildGUI extends JInternalFrame {
	...
	public BatchBuildGUI(JFrame parent, MDIDesktopPane mdiDesktopPane) {
		super("Batch Build", true, // resizable
				true, // closable
				true, // maximizable
				true);// iconifiable
	...
	private final void sizeIt(JComponent jc, Dimension min, Dimension pref,
			Dimension max) {
	...
	private final Box sized(JComponent jc, Dimension min, Dimension pref,
			Dimension max, boolean centered) {
		...
		try {
			if (!m_templateField.getText().equals("")
				...
				if (objectFormat == null) {
					...
					throw new Exception(
							"Unknown object format in template file.");
				}
				...
				int n = JOptionPane.showConfirmDialog(Administrator
						.getDesktop(), "Based on template, "
						+ "generated objects will be in "
						+ formatMap.get(objectFormat)
				...
				if (n == JOptionPane.YES_OPTION) {
					...
					try {
						...
						System.err.println("BatchBuildGUI"
								+ " frame select vetoed " + e.getMessage());
					}
					BatchThread batchThread = null;
					try {
						batchThread = new BatchThread(batchOutput, batchOutput
								.getJTextArea(), "Building Batch . . .");
					} catch (Exception e) {
						System.err.println("BatchBuildGUI"
			...
			System.err.println("BatchBuildGUI" + " general error "
					+ e.getMessage());
		}
	}
	...
	protected void pidsAction() {
		try {
			FileDialog dlg = new FileDialog(Administrator.INSTANCE,
					"PIDs Output File", FileDialog.SAVE);
			if (Administrator.batchtoolLastDir != null) {
