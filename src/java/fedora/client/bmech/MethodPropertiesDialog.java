 ...
 * 
 * <p>
 * <b>Title:</b> MethodPropertiesDialog.java
 * </p>
 * <p>
 * <b>Description:</b>
 * </p>
...
public class MethodPropertiesDialog extends JDialog {
	...
	public MethodPropertiesDialog(MethodsPane parent, String methodName,
		...
		if (builderClassName
			...
			if (parent.hasBaseURL()) {
				this.baseURL = (parent.getBaseURL().endsWith("/")) ? parent
						.getBaseURL() : (parent.getBaseURL() + "/");
			}
	...
	private JPanel setBindingPanel() {
		...
		rb_httpDS = new JRadioButton(
				"Fedora LOCAL HTTP Resolver: (enter datastream parm name)",
				false);
		...
		// http - full URL
		rb_httpFull = new JRadioButton(
				"Multi-Server Service: (enter full URL for method)", false);
		...
		URL_textRelative.setMinimumSize(new Dimension(200,
				(int) URL_textRelative.getMinimumSize().height));
		URL_textRelative
	...
	private JPanel setParmsPanel() {
		...
		if (builderClassName
			...
			parmTypes = new String[] { (String) parmTypeToDisplayTbl
		...
		String[] parmReq = (String[]) parmReqTbl.keySet()
		...
		String[] parmPassBy = (String[]) passByTbl.keySet().toArray(
				new String[0]);
	...
	private void showURLHelp() {
		...
		JOptionPane.showMessageDialog(this, js, "Help for Method Binding URL",
	...
	private boolean validMethodProperties(MethodProperties mp) {
		...
		// properties...");
		return (validBinding(mp) && validMethodParms(mp.methodParms) && validReturnTypes(mp.returnMIMETypes)) ? true
				: false;
	}
	...
	private boolean validBinding(MethodProperties mp) {
		...
		if (builderClassName
			...
			if (mp.methodFullURL == null
				...
				return false;
			} else if (!(this.parmsFromURLMissing(mp.methodFullURL,
					mp.methodParms))) {
	...
	private MethodParm[] unloadMethodParms() {
		...
		for (int i = 0; i < rowcount; i++) {
			...
			if (parmName != null && !parmName.equalsIgnoreCase("")
				...
				parm.parmType = ((String) parmTypeTbl.get(parmTable.getValueAt(
						i, 1)));
				parm.parmRequired = ((String) parmReqTbl.get(parmTable
						.getValueAt(i, 2)));
				parm.parmPassBy = ((String) passByTbl.get(parmTable.getValueAt(
						i, 3)));
				...
				if ((values = (String) parmTable.getValueAt(i, 6)) != null) {
					...
					parm.parmDomainValues = (String[]) domainValues
							.toArray(new String[0]);
				}
	...
	private void renderBindingProperties(MethodProperties properties) {
		 ...
		 * rb_httpRelative.setSelected(false);
		 * 
		 * URL_textDS.setText(""); URL_textFull.setText("");
		 * URL_textRelative.setText(""); URL_textDS.setEnabled(false);
		 * URL_textFull.setEnabled(false); URL_textRelative.setEnabled(false);
		 * 
	...
	private void assertMethodPropertiesMsg(String msg) {
		JOptionPane.showMessageDialog(this, new String(msg),
				"Method Properties Message", JOptionPane.INFORMATION_MESSAGE);
	}
}