...
import fedora.client.bmech.xml.BDefMETSSerializer;
...
public class BDefBuilder
    ...
    public static void main(String[] args) {
        try {
            if (args.length == 5) {
                ...
                frame.getContentPane().add(new BDefBuilder(protocol,
                                                           host,
                                                           port,
                                                           user,
                                                           pass,
                ...
                System.out
                        .println("BDefBuilder main method requires 5 arguments.");
                System.out
                        .println("Usage: BDefBuilder protocol host port user pass");
            }
        } catch (Exception e) {
    ...
    public BDefBuilder(String protocol,
                       String host,
                       int port,
                       String user,
                       String pass,
        ...
        if (mets != null) {
            ...
            if (chooser.showSaveDialog(tabpane) == JFileChooser.APPROVE_OPTION) {
                ...
                try {
                    ...
                    e.printStackTrace();
                    assertTabPaneMsg(("BDefBuilder: Error saving METS file for bdef: " + e
                                             .getMessage()),
                                     "BDefBuilder");
                }
            } else {
    ...
    public void ingestBDef() {
        ...
        if (mets != null) {
            try {
                ...
                e.printStackTrace();
                assertTabPaneMsg(("BDefBuilder: Error saving METS to stream for bdef: " + e
                                         .getMessage()),
                                 "BDefBuilder");
            }
            try {
                ...
                e.printStackTrace();
                assertTabPaneMsg(("BDefBuilder: error ingesting bdef object: " + e
                                         .getMessage()),
                                 null);
            }
    ...
    public BDefMETSSerializer savePanelInfo() {
        ...
        try {
            mets =
                    new BDefMETSSerializer(newBDef, dcg.getRootElement(), mmg
                            .getRootElement());
        } catch (Exception e) {
            e.printStackTrace();
            assertTabPaneMsg("BDefBuilder: error in creating METS for bdef.",
                             null);
        }
        //mets.printMETS();
        return mets;
    ...
    private boolean validGeneralTab(GeneralPane gp) {
        if (gp.rb_chosen.equalsIgnoreCase("retainPID")
            ...
            assertTabPaneMsg("Behavior Mechanism Label is missing on General Tab.",
                             gp.getName());
            return false;
        } else if (gp.getBObjectName() == null
                || gp.getBObjectName().trim().equals("")) {
            assertTabPaneMsg("Behavior Mechanism Nickname is missing on General Tab.",
                             gp.getName());
            return false;
