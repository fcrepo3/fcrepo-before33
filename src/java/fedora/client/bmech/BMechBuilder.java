...
public class BMechBuilder extends JInternalFrame {
		...
		try {
			if (args.length == 5) {
				...
				frame.getContentPane()
						.add(
								new BMechBuilder(protocol, host, port, user,
										pass, dir), BorderLayout.CENTER);
				frame.setSize(700, 500);
				frame.setVisible(true);
			} else {
				System.out
						.println("BMechBuilder main method requires 5 arguments.");
				System.out
						.println("Usage: BMechBuilder protocol host port user pass");
			}
		} catch (Exception e) {
	...
	public BMechBuilder(String protocol, String host, int port, String user,
			String pass, File dir) {
	...
	private void setListeners() {
		...
		tabpane.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				// everytime a tab changes, update the bmech template object in
				// memory
				updateBMechTemplate();
				...
				if (currentTabIndex == 3) {
					DatastreamInputPane dsip = (DatastreamInputPane) tabpane
							.getComponentAt(3);
		...
		if (mets != null) {
			...
			if (chooser.showSaveDialog(tabpane) == JFileChooser.APPROVE_OPTION) {
				...
				try {
					...
					e.printStackTrace();
					assertTabPaneMsg(
							("BMechBuilder: Error saving METS file for bmech: " + e
									.getMessage()), "BMechBuilder");
				}
			} else {
				assertTabPaneMsg(
						"BMechBuilder: You did not specify a file to Save.",
						"BMechBuilder");
			}
		}
	}
	...
	public void ingestBMech() {
		...
		if (mets != null) {
			try {
				...
				e.printStackTrace();
				assertTabPaneMsg(
						("BMechBuilder: Error saving METS to stream for bmech: " + e
								.getMessage()), "BMechBuilder");
			}
			try {
				...
				e.printStackTrace();
				assertTabPaneMsg(
						("BMechBuilder: error ingesting bmech object: " + e
								.getMessage()), null);
			}
	...
	public BMechMETSSerializer savePanelInfo() {
		...
		if (validateBMechTemplate()) {
			...
			try {
				...
				e.printStackTrace();
				assertTabPaneMsg("BMechBuilder: error serializing dc record",
						null);
			}
			try {
				...
				e.printStackTrace();
				assertTabPaneMsg(
						"BMechBuilder: error serializing service profile", null);
			}
			try {
				...
				e.printStackTrace();
				assertTabPaneMsg(
						"BMechBuilder: error serializing ds input spec", null);
			}
			try {
				...
				e.printStackTrace();
				assertTabPaneMsg("BMechBuilder: error serializing method map",
						null);
			}
			try {
			...
			try {
				mets = new BMechMETSSerializer(newBMech, dcg.getRootElement(),
						spg.getRootElement(), dsg.getRootElement(), mmg
								.getRootElement(), wsdlg.getRootElement());
			} catch (Exception e) {
				e.printStackTrace();
				assertTabPaneMsg(
						"BMechBuilder: error in creating METS for bmech.", null);
			}
			// mets.printMETS();
			// return mets;
		}
	...
	private boolean validGeneralTab(GeneralPane gp) {
		if (gp.rb_chosen.equalsIgnoreCase("retainPID")
			...
			assertTabPaneMsg("The test PID value is missing on General Tab.",
					gp.getName());
			return false;
		} else if (gp.getBDefContractPID() == null
				|| gp.getBDefContractPID().trim().equals("")) {
			...
			assertTabPaneMsg(
					"Behavior Object Description is missing on General Tab.",
					gp.getName());
			return false;
		} else if (gp.getBObjectName() == null
				|| gp.getBObjectName().trim().equals("")) {
			assertTabPaneMsg(
					"Behavior Object Name (1-word) is missing on General Tab.",
					gp.getName());
			return false;
		} else if (gp.getDCElements().length <= 0) {
			assertTabPaneMsg(
					"You must enter at least one DC element on General Tab.",
					gp.getName());
			return false;
		}
		return true;
	...
	private boolean validProfileTab(ServiceProfilePane spp) {
		if (spp.getServiceName() == null) {
			...
			assertTabPaneMsg(new String(
					"You must enter the messaging protocol for"
							+ " this service in the Service Profile Tab"), spp
					.getName());
			return false;
		} else if (spp.getOutputMIMETypes().length == 0) {
			assertTabPaneMsg(new String(
					"You must enter at least one output MIME type"
	...
	private boolean validMethodsTab(MethodsPane mp) {
		if (mp.hasBaseURL()
			...
			assertTabPaneMsg(
					"You must enter at least one method definition in Service Methods Tab.",
					mp.getName());
			return false;
		} else {
			Method[] methods = mp.getMethods();
			for (int i = 0; i < methods.length; i++) {
				if (methods[i].methodProperties == null) {
					assertTabPaneMsg(new String(
							"You must enter properties for the method "
									+ methods[i].methodName)
							+ " in the Service Methods Tab", mp.getName());
					return false;
				} else if (!methods[i].methodProperties.wasValidated) {
					assertTabPaneMsg(new String(
							"You must enter valid properties for the method "
									+ methods[i].methodName
									+ " in the Service Methods Tab"), mp
							.getName());
					return false;
				}
			}
	...
	private boolean validDSInputTab(DatastreamInputPane dsp) {
		...
		if (bindkeys.size() != rules.length) {
			assertTabPaneMsg(new String(
					"You have not completed entry of the Datastream"
							+ " input binding rules"
		...
		for (int i = 0; i < rules.length; i++) {
			if (rules[i].bindingKeyName == null) {
				assertTabPaneMsg(
						new String(
								"A Datastream parm name is missing"
	...
	private boolean validDocsTab(DocumentsPane docp) {
		...
		for (int i = 0; i < docs.length; i++) {
			if (docs[i].dsLabel == null) {
				assertTabPaneMsg(new String(
						"You must enter a Label for all documents"
								+ "listed on the Documents Tab."), docp
						.getName());
				return false;
			} else if (docs[i].dsMIMEType == null) {
				assertTabPaneMsg(new String(
						"You must enter a MIME type for all documents"
								+ "listed on the Documents Tab."), docp
						.getName());
				return false;
			}
	...
	private void showGeneralHelp() {
		...
		JOptionPane.showMessageDialog(this, helptxt, "Help for General Tab",
	...
	private void showMethodsHelp() {
		...
		JOptionPane.showMessageDialog(this, helptxt,
				"Help for Service Methods Tab", JOptionPane.OK_OPTION);
	}
	...
	private void showDatastreamsHelp() {
		...
		JOptionPane.showMessageDialog(this, helptxt,
				"Help for Datastream Input Tab", JOptionPane.OK_OPTION);
	}
	...
	private void showDocumentsHelp() {
		...
		JOptionPane.showMessageDialog(this, helptxt, "Help for Documents Tab",
				JOptionPane.OK_OPTION);
	}
	...
	private void showProfileHelp() {
		...
		JOptionPane.showMessageDialog(this, helptxt,
				"Help for Service Profile Tab", JOptionPane.OK_OPTION);
	}
	...
	private void printBMech() {
		...
		System.out.println("serviceName: "
				+ newBMech.getServiceProfile().serviceName);
		System.out.println("serviceLabel: "
				+ newBMech.getServiceProfile().serviceLabel);
		System.out.println("serviceTestURL: "
				+ newBMech.getServiceProfile().serviceTestURL);
		...
		for (int i = 0; i < sw.length; i++) {
			System.out.println(">>> " + sw[i].swName + "," + sw[i].swType + ","
					+ sw[i].swVersion + "," + sw[i].swLicenceType + ",");
		}
		...
		for (int i = 0; i < rules.length; i++) {
			System.out.println(">>>name= " + rules[i].bindingKeyName + "\n"
					...
					+ rules[i].maxNumBindings + "\n" + ">>>order= "
					+ rules[i].ordinality + "\n" + ">>>label= "
					+ rules[i].bindingLabel + "\n" + ">>>instruct= "
					+ rules[i].bindingInstruction + "\n");
		}
	}
}