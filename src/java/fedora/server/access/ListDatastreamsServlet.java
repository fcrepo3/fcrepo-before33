...
public class ListDatastreamsServlet extends HttpServlet {
    ...
    private static final Logger LOG = Logger.getLogger(
            ListDatastreamsServlet.class.getName());
	...
	public void listDatastreams(Context context, String PID, Date asOfDateTime,
		...
		try {
			...
			new DatastreamDefSerializerThread(context, PID, dsDefs,
	...
	public class DatastreamDefSerializerThread extends Thread {
		 ...
		 */
		public DatastreamDefSerializerThread(Context context, String PID,
			...
			if (pw != null) {
				try {
					...
					for (int i = 0; i < dsDefs.length; i++) {
						pw
								.write("    <datastream " + "dsid=\""
										+ StreamUtility.enc(dsDefs[i].dsID)
										+ "\" " + "label=\""
										+ StreamUtility.enc(dsDefs[i].dsLabel)
										+ "\" " + "mimeType=\""
										+ StreamUtility.enc(dsDefs[i].dsMIME)
										+ "\" />");
					}
