...
//    }
    ...
    public MethodDef[] listMethods(String bDefPID, BMechReader bmechreader, Date versDateTime)
            throws MethodNotFoundException, ServerException 
    {
        if ( bDefPID.equalsIgnoreCase("fedora-system:1") ||
             bDefPID.equalsIgnoreCase("fedora-system:3"))
...
//    }
            ...
            throws ServerException 
    {
        ...
        RelationshipTuple cmPIDs[] = getRelationships(null, Constants.RELS_EXT.HAS_FORMAL_CONTENT_MODEL.uri);
...
//                if (diss!=null) 
//                {
//                    MethodDef[] methods=listMethods(diss.bDefID, versDateTime);
//                    if (methods!=null) 
//                    {
...
//            }
    ...
    public RelationshipTuple[] getRelationships(String subjectURI, String relationship) throws ServerException
    {
            ...
            if (relsExt != null) 
            {
                ...
                try 
                {
                    ...
                    while (iter.hasNext() && !done) 
                    {
                        ...
                        if ((relationship == null || triple.getPredicate().toString().endsWith(relationship)) &&
                            (subjectURI == null || subjectURI.matches(triple.getSubject().toString())   ))                              
                        {
                            relsExtDataTmp.add(triple);
                        }
                    }               
                }
        ...
        {
            int size = relsExtDataTmp.size();
            if (size > 0)
            {
                relsExtData = new RelationshipTuple[size];
            }
            for (int i = 0; i < size; i++)
            {
                Triple triple = (Triple)(relsExtDataTmp.get(i));
                ...
                if (object instanceof Literal)
                {
                    ...
                    literalType = ((Literal)object).getDatatypeURI().toString();
                 }
                else
                {
                ...
                relsExtData[i] = new RelationshipTuple(this.GetObjectPID(),
                                                       triple.getPredicate().toString(), 
                                                       objURI, objLiteral, literalType);
      //          if (relsExtData[i].objectURI.startsWith("info:fedora/"))
      //          {
      //              relsExtData[i].objectPID = relsExtData[i].objectPID.substring(12);
      //          }                
            }
            relsExtDataTmp = null;
        }
