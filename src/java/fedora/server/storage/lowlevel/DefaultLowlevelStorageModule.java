...
public class DefaultLowlevelStorageModule extends Module implements ILowlevelStorage {	
	...
	protected Map getModuleParameters() throws ModuleInitializationException {
		...
		String objectStoreBase = getModuleParameter(DefaultLowlevelStorage.OBJECT_STORE_BASE, true);
		String datastreamStoreBase = getModuleParameter(DefaultLowlevelStorage.DATASTREAM_STORE_BASE, true);
		...
		String param = getModuleParameter("backslash_is_escape", false).toLowerCase();
		...
		configuration.put(DefaultLowlevelStorage.OBJECT_STORE_BASE, objectStoreBase);
		configuration.put(DefaultLowlevelStorage.DATASTREAM_STORE_BASE, datastreamStoreBase);
	...
	protected String getModuleParameter(String parameterName, boolean parameterAsAbsolutePath) throws ModuleInitializationException {
		String parameterValue = getParameter(parameterName, parameterAsAbsolutePath);
		
		if (parameterValue == null) {
			throw new ModuleInitializationException(parameterName + " parameter must be specified", getRole());
		}
