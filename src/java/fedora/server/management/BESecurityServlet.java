...
/**
 * Servlet for editing the config/beSecurity.xml file in a user-friendly way. A
 * GET request will present an HTML form populated with existing values in
 * beSecurity.xml and non-configured values for all other bMechs and methods
 * that exist in the repository at the time of the request. A POST request will
 * save the results of the form submission over the existing
 * config/beSecurity.xml file, cause an automatic reload of the policies, then
 * describe to the user how/why they may change the servlet password database to
 * reflect the changes.
 * 
 * @author Chris Wilper
 */
public class BESecurityServlet
    ...
    /**
     * If true, changes won't be saved on POST. Instead, an XML document showing
     * the servlet parameters and the abbreviated and full beSecurity.xml file
     * will be returned.
     */
    ...
    /**
     * Respond to an HTTP GET request. Displays an html form for editing backend
     * security configuration, or an xml document providing enough information
     * to construct the form (if xml=true).
     */
    @Override
    public void doGet(HttpServletRequest req, HttpServletResponse res)
        ...
        try {
            ...
            if (xml != null && xml.equals("true")) {
                ...
                pw.flush();
                Reader xmlReader =
                        new InputStreamReader(new ByteArrayInputStream(memOut
                                .toByteArray()));
    ...
    /**
     * Respond to an HTTP POST request. This will overwrite the existing
     * config/beSecurity.xml file based on the parameters passed in, re-generate
     * the policies, then inform the user of what needs to be done to
     * tomcat-users_fedoraTemplate.xml.
     */
    @Override
        ...
        try {
            ...
            if (_DEBUG) {
                ...
                synchronized (m_configFile) {
                    ...
                    BESecurityConfig originalConfig =
                            BESecurityConfig
                                    .fromStream(new FileInputStream(m_configFile));
                    ...
                    try {
                        ...
                        Context context =
                                ReadOnlyContext
                                        .getContext(Constants.HTTP_REQUEST.REST.uri,
                                                    req);
                        ...
                        throw new Exception("Backend policy generation failed. "
                                                    + msg,
                                            th);
                    }
                }
            ...
            try {
                th.printStackTrace();
                res.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, th
                        .getMessage());
            } catch (Exception ex) {
            }
        } finally {
    ...
    private static void writeSavedHTML(PrintWriter writer,
        ...
        writer
                .println("<html><head><title>Saved Backend Security Configuration</title></head><body>");
        writer.println("<table border=\"0\" width=\"850\">");
        writer.println("    <tr>");
        writer
                .println("       <td><img src=\"/fedora/images/newlogo2.jpg\" width=\"70\" height=\"67\"></td>");
        writer.println("       <td valign=\"top\">");
        writer
                .println("          <center><span style=\"font-weight: bold; color: #000000; margin-top: 4px; margin-bottom: 4px; font-size: 24px; line-height: 110%; padding-top: 8px; padding-bottom: 4px;\">Backend Security Configuration<br/>[Changes Saved]</span>");
        writer.println("             </nobr>");
        ...
        writer
                .println("  <li> Your changes have been successfully saved.</li>");
        writer
                .println("  <li> Backend policies have been automatically regenerated and applied.</li>");
        writer
                .println("  <li> To go back to the form, <a href=\"backendSecurity\">click here</a>.</li>");
        writer.println("</ul></p>");
        writer.println("<h3>Note:</h3>");
        writer
                .println("<p>If you have configured any backend services to authenticate to Fedora using");
        writer
                .println("basic authentication, you will also need to manually edit the tomcat <code>conf/tomcat-users_fedoraTemplate.xml</code>");
        writer.println("file and restart Fedora.</p>");
        writer
                .println("<p>For each service role there must be a user and password");
        writer
                .println("configured in this file so that Fedora can properly authenticate the service.</p>");
        writer
                .println("<p>For example, if <code>demo:13</code> and <code>demo:2/getHigh</code> must authenticate");
        writer
                .println("to Fedora, the file must be edited to include the following:</p>");
        writer.println("<pre>");
        writer
                .println("  &lt;user name=\"demo13user\" password=\"demo13pass\" roles=\"fedoraRole=demo:13\"/&gt;");
        writer
                .println("  &lt;user name=\"demo2user\" password=\"demo2pass\" roles=\"fedoraRole=demo:2/getHigh\"/&gt;");
        writer.println("</pre>");
        writer
                .println("<p>The actual usernames and password will be of your own choosing, ");
        writer
                .println("and should match the credentials that the backend service is configured");
        writer.println("to authenticate to Fedora with during callbacks.</p>");
        writer.println("</body></html>");
    }

    /**
     * For debugging purposes, write an XML document detailing the servlet input
     * parameters and the beSecurity.xml file that would result from them.
     */
    ...
    private static BESecurityConfig getConfig(Map params) {
        ...
        while (pids.hasNext()) {
            ...
            BMechRoleConfig bMechConfig =
                    new BMechRoleConfig(defaultConfig, pid);
            ...
            for (int i = 0; i < methodNames.size(); i++) {
                ...
                MethodRoleConfig methodConfig =
                        new MethodRoleConfig(bMechConfig, methodName);
     ...
     */
    private static void setValues(BERoleConfig config, Map params) {
        config.setCallSSL(getBoolean(params, config.getRole() + "/callSSL"));
        config.setCallBasicAuth(getBoolean(params, config.getRole()
                + "/callBasicAuth"));
        config.setCallUsername(getString(params, config.getRole()
                + "/callUsername"));
        config.setCallPassword(getString(params, config.getRole()
                + "/callPassword"));
        config.setCallbackSSL(getBoolean(params, config.getRole()
                + "/callbackSSL"));
        config.setCallbackBasicAuth(getBoolean(params, config.getRole()
                + "/callbackBasicAuth"));
        config.setIPList(getStringArray(params, config.getRole() + "/iplist"));
    }

    /**
     * Get the given value out of the map as a String. If there is no such
     * value, or the value is empty, return null.
     */
    ...
    /**
     * Get the given value out of the map as a Boolean. If there is no such
     * value, or the value is empty, return null.
     */
    ...
    /**
     * Get the given value out of the map as an array of Strings. If there is no
     * such value, or the value is empty, return null. Otherwise, return the
     * array with one element for each space-delimited token.
     */
    ...
    /**
     * Get a map of all BMechs and their associated lists of method names as
     * they currently exist in the repository.
     */
    private Map getAllBMechMethods(Context context) throws Exception {
        Map<String, List<String>> map = new HashMap<String, List<String>>();
        String[] resultFields = new String[] {"pid"};
        FieldSearchQuery query =
                new FieldSearchQuery(Condition.getConditions("fType=M"));
        FieldSearchResult result =
                m_fieldSearch.findObjects(resultFields, 100, query);
    ...
    /**
     * Get the list of method names for the given bMech as it currently exists
     * in the repository.
     */
    private List<String> getMethods(String bMechPID, Context context)
            throws Exception {
        List<String> list = new ArrayList<String>();
        BMechReader reader =
                m_doManager.getBMechReader(false, context, bMechPID);
        MethodDef[] defs = reader.getServiceMethods(null);
        for (MethodDef element : defs) {
            list.add(element.methodName);
        }
        return list;
    }

    /**
     * Initialize the servlet by getting a reference to the required modules of
     * the running Fedora instance and making sure the beSecurity.xml and
    ...
    public void init() throws ServletException {
        try {
            ...
            // config file
            m_configFile = new File(fedoraHome, "server/config/beSecurity.xml");
