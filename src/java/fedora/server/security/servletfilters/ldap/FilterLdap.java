...
public class FilterLdap extends BaseCaching {
	...
	public void destroy() {
		String m = FilterSetup.getFilterNameAbbrev(this.FILTER_NAME) 
            + " destroy() ";
		try {
			log.debug(m + ">");
			super.destroy();
	...
	protected void initThisSubclass(String key, String value) {
    	...
    	try {
    		...
    		if (VERSION_KEY.equals(key)) {
    			...
    			REQUIRE_RETURNED_ATTRS = Boolean.valueOf(value);
    			setLocally = true;
                */		    			
        	} else {
	...
	private boolean individualUserBind() {
		boolean individualUserBind = (bindRequired() && AUTHENTICATE 
            && ((PASSWORD == null) || "".equals(PASSWORD)));
		return individualUserBind;
	}

	private boolean individualUserComparison() {
		boolean individualUserComparison = (AUTHENTICATE 
            && (PASSWORD != null) && ! "".equals(PASSWORD));
	...
	private final SearchControls getSearchControls() {
		...
		try {
			...
			boolean retobj = true;
			boolean deref = true;
			searchControls = new SearchControls(SearchControls.SUBTREE_SCOPE,
				nEntries2return,
	...
	private NamingEnumeration getBasicNamingEnumeration(String userid, 
            String password, String filter, SearchControls searchControls, 
            Hashtable env) 
    ...
    private NamingEnumeration getNamingEnumeration(String userid, 
                ...
                if (USE_FILTER.equalsIgnoreCase(PW_NULL)) {
                	...
                	if (AUTHENTICATE) {
	                    ...
	                    if (UNAUTHENTICATE_USER_UNCONDITIONALLY.equalsIgnoreCase(PW_NULL)) {
	                        ...
	                        assert true : "bad value for PW_NULL==" 
	                            + PW_NULL;
	                    }
                	}
                }
                ...
                if (USE_FILTER.equalsIgnoreCase(PW_0)) {
                	...
                	if (AUTHENTICATE) {
	                    ...
	                    if (UNAUTHENTICATE_USER_UNCONDITIONALLY.equalsIgnoreCase(PW_0)) {
	                        ...
	                        assert true : "bad value for PW_0==" 
	                            + PW_0;
	                    }
                	}
                }
        ...
        NamingEnumeration ne = null;
        try {
            ne = getBasicNamingEnumeration(userid, password, filter, 
                    searchControls, env);
            assert ne != null;
	...
	private static Boolean comparePassword(Attributes attributes, 
            String password, String passwordAttribute) 
	...
	private void getAttributes(Attributes attributes, Map map) 
            throws Throwable {
		String m = FilterSetup.getFilterNameAbbrev(this.FILTER_NAME) 
            + " getAttributes() ";
		log.debug(m + ">");
		try {
			for (int i = 0; i < ATTRIBUTES2RETURN.length; i++) {
				...
				if ((GROUPS_NAME != null) && ! "".equals(GROUPS_NAME)) {
					key = GROUPS_NAME;
			    	log.debug(m + "values collected and interpreted as groups==" 
                        + key);
				}
				Set values;
	...
	private Boolean processNamingEnumeration(NamingEnumeration ne, 
            String password, Boolean authenticated, Map map) { 
		String m = FilterSetup.getFilterNameAbbrev(this.FILTER_NAME) 
	...
	public void populateCacheElement(CacheElement cacheElement, 
    	...
    	try { 
            ...
            try {
        	    ne = getNamingEnumeration(cacheElement.getUserid(), 
                        password, filter, searchControls, env);	
                assert ne != null;
                ...
                if (AUTHENTICATE && individualUserBind() 
                    ...
                    assert map.isEmpty();
                    try {
                        authenticated = processNamingEnumeration(ne, password, authenticated, 
                            map);
                        
                        
                        log.debug(m + "back from pNE.  AUTHENTICATE==" + AUTHENTICATE + " authenticated==" + authenticated + " map==" + map);
