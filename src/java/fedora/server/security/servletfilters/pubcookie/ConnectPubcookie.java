...
//import org.apache.commons.httpclient.CookiePolicy;
...
public class ConnectPubcookie {	
	...
	public final Cookie[] getResponseCookies() {
		log.debug(this.getClass().getName() + ".getResponseCookies() " + "cookies are:");		
		for (int i=0; i<responseCookies.length; i++) {
			log.debug(this.getClass().getName() + ".getResponseCookies() " + "cookie==" + responseCookies[i]);		
		}
	...
	public final void connect(String urlString, Map requestParameters, Cookie[] requestCookies, String truststoreLocation, String truststorePassword) {
		log.debug(this.getClass().getName() + ".connect() " + " url==" + urlString + " requestParameters==" + requestParameters + " requestCookies==" + requestCookies);		
		...
		*/
		...
		try {
			...
			log.debug(this.getClass().getName() + ".connect() " + "(with configured url) statusCode==" + statusCode);
		} catch (Exception e) {
			...
			if (redirectHeader != null) {
				...
				if (redirectString != null) {
					...
					try {
						client.executeMethod(method);
						statusCode = method.getStatusCode();
						log.debug(this.getClass().getName() + ".connect() " + "(on redirect) statusCode==" + statusCode);
					} catch (Exception e) {
		...
		if (statusCode == 200) { // this is either the original, non-302, status code or the status code after redirect
	        ...
	        try {
				...
				for (int i=0; i<responseCookies2.length; i++) {
					log.debug(this.getClass().getName() + ".connect() header==" + responseCookies2[i]);
				}
				...
				if (t.getCause() != null) log.error(this.getClass().getName() + ".connect() cause==" + t.getCause().getMessage());
	        }
			completedFully = true;
