...
public class FilterPubcookie extends BaseCaching {
	    ...
	    for (int i = 0; i < children.getLength(); i++) {
    		...
    		if ("input".equalsIgnoreCase(tag)) {
    			...
    			if ("hidden".equalsIgnoreCase(type) && value != null) {
    				if (log.isDebugEnabled()) log.debug(format("capturing hidden fields", name, value));								
	...
	public void populateCacheElement(CacheElement cacheElement, String password) {
		...
		if (log.isDebugEnabled()) {
			...
			log.debug(format(method, null, "PUBCOOKIE_LOGINPAGE_URL", PUBCOOKIE_LOGINPAGE_URL));
		}
		tidyConnect.connect(PUBCOOKIE_LOGINPAGE_URL, NO_REQUEST_PARAMETERS, NO_COOKIES, TRUSTSTORE_LOCATION, TRUSTSTORE_PASSWORD);
		if (! tidyConnect.completedFully()) {
			if (log.isInfoEnabled()) log.info(format(method,"form page did not load"));								
		} else {
			Cookie[] formpageCookies = tidyConnect.getResponseCookies();
			Node formpageDocument = tidyConnect.getResponseDocument();
			String action = getAction(formpageDocument, PUBCOOKIE_LOGINPAGE_FORM_NAME);
			if (log.isDebugEnabled()) log.debug(format(method, "action", action));								
			...
			if (log.isDebugEnabled()) {
				...
				while (iter.hasNext()) {
					...
					log.debug(format(method, null, key, (String) formpageFields.get(key)));												
				}
			}
		
			formpageFields.put(PUBCOOKIE_LOGINPAGE_INPUT_NAME_USERID, cacheElement.getUserid());
			formpageFields.put(PUBCOOKIE_LOGINPAGE_INPUT_NAME_PASSWORD, password);

			if (log.isDebugEnabled()) {
				iter = formpageFields.keySet().iterator();
				while (iter.hasNext()) {
					String key = (String) iter.next();
					log.debug(format(method, " form field after", key, (String) formpageFields.get(key)));												
				}
			}
			
			tidyConnect = new ConnectPubcookie();
			if (log.isDebugEnabled()) log.debug(format(method, "b4 second connect()"));								
			tidyConnect.connect(action, formpageFields, formpageCookies, TRUSTSTORE_LOCATION, TRUSTSTORE_PASSWORD);
			if (! tidyConnect.completedFully()) {
				if (log.isDebugEnabled())log.debug(format(method, "result page did not load"));								
			} else {
				Cookie[] resultpageCookies = tidyConnect.getResponseCookies();
				if (log.isDebugEnabled()) log.debug(format(method, " cookies receieved", "n", Integer.toString(resultpageCookies.length)));								
				for (int i = 0; i < resultpageCookies.length; i++) {
					Cookie cookie = resultpageCookies[i];
					if (log.isDebugEnabled()) {
						log.debug(format(method, "another cookie", "cookie name" + cookie.getName()));								
						log.debug(format(method, "another cookie", "length", Integer.toString(cookie.getName().length())));
					}
					if (PUBCOOKIE_NAME.equals(cookie.getName())) {
						if (log.isInfoEnabled()) log.debug(format(method, " found pubcookie login cookie"));								
				...
				if (authenticated == null) {
					...
					if (! authenticated.booleanValue()) {
						if (log.isDebugEnabled()) log.debug(format(method, "didn't find a pubcookie login cookie"));
					}
				}
			}
