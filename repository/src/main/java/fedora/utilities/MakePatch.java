...
public class MakePatch {
    ...
    private final boolean m_beVerbose;
    ...
    public MakePatch(File originBinDir,
                     File originSrcDir,
                     String originVer,
                     File targetBinDir,
                     File targetSrcDir,
        ...
        getDiffs(new File(originBinDir, "client"),
                 ...
                 clientPatchList,
                 "");
        makeJar(new File(destDir, "fedora-" + originVer + "-client-to-"
                + targetVer + "-patch.jar"), targetClientDir, clientPatchList);
        print("File 2/3 - Server");
        ArrayList serverPatchList = new ArrayList();
        File targetServerDir = new File(targetBinDir, "server");
        getDiffs(new File(originBinDir, "server"),
                 targetServerDir,
                 serverPatchList,
                 "");
        makeJar(new File(destDir, "fedora-" + originVer + "-server-to-"
                + targetVer + "-patch.jar"), targetServerDir, serverPatchList);
        print("File 3/3 - Source");
        ArrayList sourcePatchList = new ArrayList();
        getDiffs(originSrcDir, targetSrcDir, sourcePatchList, "");
        makeJar(new File(destDir, "fedora-" + originVer + "-src-to-"
                + targetVer + "-patch.jar"), targetSrcDir, sourcePatchList);
        print("Done.");
    }

    public void makeJar(File outFile, File inDir, List fList) throws Exception {
        JarOutputStream out =
                new JarOutputStream(new FileOutputStream(outFile));
    ...
    public void getDiffs(File from, File to, ArrayList out, String prependPath)
        ...
        for (String element : toNames) {
            File toFile = new File(to, element);
            if (toFile.isDirectory()) {
                getDiffs(new File(from, element), toFile, out, prependPath
                        + element + "/");
            } else {
                if (!fromIsDir) {
                    // all files must be added since its a new dir
                    String a = prependPath + element;
                    verbose("Adding file because its dir is new: " + a);
                    out.add(a);
                } else {
                    // check for existence of each file
                    File fromFile = new File(from, element);
                    if (!fromFile.exists()) {
                        String a = prependPath + element;
                        ...
                        if (filesDiffer(fromFile, toFile)) {
                            ...
                            if (diffPrompt) {
                                System.out
                                        .println("File "
                                                + a
                                                + " has changed.  Include in patch? [y]es/no ");
                                r = br.readLine();
                            }
                            if (!diffPrompt || r.equals("")
                                    || r.toUpperCase().startsWith("y")) {
                                verbose("Adding file because it differs: " + a);
                                out.add(a);
                            } else {
                                verbose("Skipped " + a
                                        + " even though it differs.");
                            }
                        } else {
                            /*
    ...
    public boolean filesDiffer(File a, File b) throws Exception {
        return !getChecksum(a).equals(getChecksum(b));
    }

    public String getChecksum(File f) throws Exception {
    ...
    public static String getFedoraVersion(File dir, boolean binDist)
        ...
        if (binDist) {
            if (!dir.exists()) {
                throw new IOException("Provided directory, "
            ...
            if (!foundClient) {
                throw new IOException("Cannot find client dir in "
                        + dir.getAbsolutePath());
            }
            if (!foundServer) {
                throw new IOException("Cannot find server dir in "
                        + dir.getAbsolutePath());
            }
        }
        ...
        if (binDist) {
            props
                    .load(new FileInputStream(new File(dir,
                                                       "server/tomcat41/webapps/fedora/WEB-INF/classes/fedora/server/resources/Server.properties")));
        } else {
            props
                    .load(new FileInputStream(new File(dir,
                                                       "repository/src/main/resources/properties/server/fedora/server/resources/Server.properties")));
        }
    ...
    public static void main(String[] args) {
        try {
            System.out
                    .println("----------------------------------------------");
            System.out
                    .println("MakePatch - Build a Fedora distribution patch.");
            System.out
                    .println("----------------------------------------------");
            System.out
                    .println("This program will create patch files to go from one complete");
            System.out.println("Fedora distribution to another.\n");
            System.out
                    .println("Before continuing, you should have two complete");
            System.out
                    .println("Fedora distributions (client, server, and full source for each)");
            System.out
                    .println("unpacked in directories that you can provide to this program.\n");
            ...
            if (r.equals("") || r.toUpperCase().startsWith("Y")) {
                ...
                //
                System.out
                        .println("Enter the FEDORA_HOME directory of the origin binary distribution.");
                System.out
                        .println("(this is the directory with the client/ and server/ directories)");
                System.out.println("Origin Binary Distribution Directory: ");
                File originBinDir = new File(br.readLine());
                String originBinVer =
                        MakePatch.getFedoraVersion(originBinDir, true);
                System.out.println("Appears to be a Fedora v" + originBinVer
                        + " binary distribution, ok.\n");
                System.out
                        .println("Enter the installation directory of the origin source distribution.");
                System.out
                        .println("(this is the directory with the build.xml file)");
                System.out.println("Origin Source Distribution Directory: ");
                File originSrcDir = new File(br.readLine());
                String originSrcVer =
                        MakePatch.getFedoraVersion(originSrcDir, false);
                if (!originSrcVer.equals(originBinVer)) {
                    throw new IOException("Origin binary version is "
                            + originBinVer + ", but origin source version is "
                            + originSrcVer);
                }
                System.out
                        .println("Origin source and binary distributions match, good.\n");
                //
                ...
                String targetBinVer =
                        MakePatch.getFedoraVersion(targetBinDir, true);
                ...
                System.out.println("Appears to be a Fedora v" + targetBinVer
                        + " binary distribution, ok.\n");
                System.out.println("Target Source Distribution Directory: ");
                File targetSrcDir = new File(br.readLine());
                String targetSrcVer =
                        MakePatch.getFedoraVersion(targetSrcDir, false);
                if (!targetSrcVer.equals(targetBinVer)) {
                    throw new IOException("Target binary version is "
                            + targetBinVer + ", but target source version is "
                            + targetSrcVer);
                }
                System.out
                        .println("Target source and binary distributions match, good.\n");
                //
                // Get output dir
                //
                System.out
                        .println("Enter the directory where the patch files should be written.");
                ...
                System.out.println("\nAbout to create patch files for Fedora v"
                        + originSrcVer + " to v" + targetSrcVer + " upgrade.");
                System.out
                        .println("Want to be prompted before adding items that differ? [y]es/no ");
                r = br.readLine();
                ...
                System.out
                        .println("Re-run this program when you have the information.");
            }
        } catch (Exception e) {
