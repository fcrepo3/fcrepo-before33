...
public class METSLikeExportDOSerializer
    ...
    public void serialize(DigitalObject obj, OutputStream out, String encoding)
        ...
        if (s_localServerDissemUrlStart==null) {
			...
			// ************************PATCH CODE**************************
			// set patterns to detect configured host:port of the repository
			...
			if ( obj.getFedoraObjectType()==DigitalObject.FEDORA_BMECH_OBJECT &&
				 (ds.DatastreamID.equals("SERVICE-PROFILE") || 
				  ds.DatastreamID.equals("WSDL")) ) {
					// convert instances of the local repository host:port 
					// to the special string "fedora.local.server" to support
					// migration of relative repository URLs into new repository.
					buf.append(makeFedoraLocalURLs(new String(ds.xmlContent, "UTF-8")));
			} else {
        ...
        while (iter.hasNext()) {
            ...
            if (!ds.DSControlGrp.equals("X")) {
                ...
                while (contentIter.hasNext()) {
					...
					// ************************PATCH CODE**************************
					if (dsc.DSControlGrp.equals("E") || dsc.DSControlGrp.equals("R")) {
						// convert instances of the local repository host:port 
						// to the special string "fedora.local.server" to support
						// migration of relative repository URLs into new repository.
						dsc.DSLocation=makeFedoraLocalURLs(dsc.DSLocation);
					}                    
					// *********************END PATCH CODE**************************
	 ...
	 * (http://local.fedora.server/..."). Examples:
	 *
	 *
	 * 	 "http://myrepo.com:8080/fedora/get/demo:1/bdef:1/getFoo"
	 *        is converted to
	 * 	      "http://local.fedora.server/fedora/get/demo:1/bdef:1/getFoo"
	 *
	 * 	 "http://myrepo.com:8080/saxon..." (internal service in bMech WSDL)
	 *        is converted to
	 * 	      "http://local.fedora.server/saxon..."
	 * @param input
	 * @return  String with relative repository URLs converted to
	 *          the Fedora local URL placeholder syntax.
	 */
