...
{
	private static Logger log = Logger.getLogger(ListMethodsFilter.class.getName());
	 ...
	 * @throws PEPException
	 */
	public ListMethodsFilter() throws PEPException
	{
		super();
	}
			...
			ServletException
	{
		...
		try
		{
			...
			actions.put(Constants.ACTION.API.getURI(), new StringAttribute(Constants.ACTION.APIA.getURI()
					.toASCIIString()));
			actions.put(Constants.ACTION.ID.getURI(), new StringAttribute(Constants.ACTION.LIST_METHODS.getURI()
					.toASCIIString()));

			req = getContextHandler().buildRequest(getSubjects(request), actions, resAttr, getEnvironment(request));

			LogUtil.statLog(request.getRemoteUser(), Constants.ACTION.LIST_METHODS.getURI().toASCIIString(), pid, null);
		}
		catch (Exception e)
		{
