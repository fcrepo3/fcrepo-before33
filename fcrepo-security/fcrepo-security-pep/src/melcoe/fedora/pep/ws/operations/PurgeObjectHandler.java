 ...
 *
 */
public class PurgeObjectHandler extends AbstractOperationHandler
{
	private static Logger log = Logger.getLogger(PurgeObjectHandler.class.getName());

	public PurgeObjectHandler() throws PEPException
	{
		super();
	}
	...
	{
		log.debug("PurgeObjectHandler/handleRequest!");
		...
		try
		{
			...
			actions.put(Constants.ACTION.ID.getURI(), new StringAttribute(Constants.ACTION.PURGE_OBJECT.getURI().toASCIIString()));
			...
			LogUtil.statLog(context.getUsername(), Constants.ACTION.PURGE_OBJECT.getURI().toASCIIString(), pid, null);
		}
		catch (Exception e)
		{
