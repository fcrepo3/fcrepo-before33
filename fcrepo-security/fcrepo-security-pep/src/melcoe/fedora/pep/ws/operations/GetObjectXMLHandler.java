 ...
 *
 */
public class GetObjectXMLHandler extends AbstractOperationHandler
{
	private static Logger log = Logger.getLogger(GetObjectXMLHandler.class.getName());

	public GetObjectXMLHandler() throws PEPException
	{
		super();
	}
	...
	{
		log.debug("GetObjectXMLHandler/handleRequest!");
		...
		try
		{
			...
			actions.put(Constants.ACTION.ID.getURI(), new StringAttribute(Constants.ACTION.GET_OBJECT_XML.getURI().toASCIIString()));
			...
			LogUtil.statLog(context.getUsername(), Constants.ACTION.GET_OBJECT_XML.getURI().toASCIIString(), pid, null);
		}
		catch (Exception e)
		{
