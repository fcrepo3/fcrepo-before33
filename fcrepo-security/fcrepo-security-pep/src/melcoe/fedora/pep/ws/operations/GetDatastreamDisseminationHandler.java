/*
 * File: GetDatastreamDisseminationHandler.java
 *
 ...
 *
 */
public class GetDatastreamDisseminationHandler extends AbstractOperationHandler
{
	private static Logger log = Logger.getLogger(GetDatastreamDisseminationHandler.class.getName());

	public GetDatastreamDisseminationHandler() throws PEPException
	{
		super();
	}
	...
	{
		log.debug("GetDatastreamDisseminationHandler/handleRequest!");
		...
		try
		{
			...
			actions.put(Constants.ACTION.ID.getURI(), new StringAttribute(Constants.ACTION.GET_DATASTREAM_DISSEMINATION.getURI().toASCIIString()));
			...
			LogUtil.statLog(context.getUsername(), Constants.ACTION.GET_DATASTREAM_DISSEMINATION.getURI().toASCIIString(), pid, dsID);
		}
		catch (Exception e)
		{
