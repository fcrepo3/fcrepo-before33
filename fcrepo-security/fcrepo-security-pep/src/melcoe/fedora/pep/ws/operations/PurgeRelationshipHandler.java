 ...
 *
 */
public class PurgeRelationshipHandler extends AbstractOperationHandler
{
	private static Logger log = Logger.getLogger(PurgeRelationshipHandler.class.getName());

	public PurgeRelationshipHandler() throws PEPException
	{
		super();
	}
	...
	{
		log.debug("PurgeRelationshipHandler/handleRequest!");
		...
		try
		{
			...
			actions.put(Constants.ACTION.ID.getURI(), new StringAttribute(Constants.ACTION.PURGE_RELATIONSHIP.getURI().toASCIIString()));
			...
			LogUtil.statLog(context.getUsername(), Constants.ACTION.PURGE_RELATIONSHIP.getURI().toASCIIString(), pid, null);
		}
		catch (Exception e)
		{
