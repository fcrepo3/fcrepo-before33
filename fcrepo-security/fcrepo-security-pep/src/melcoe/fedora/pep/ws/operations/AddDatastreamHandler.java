...
{
	private static Logger log = Logger.getLogger(AddDatastreamHandler.class.getName());

	public AddDatastreamHandler() throws PEPException
	{
		super();
	}
	...
	public RequestCtx handleRequest(MessageContext context) throws OperationHandlerException
	{
		...
		try
		{
			...
			if (controlGroup != null && !"".equals(controlGroup)) resAttr.put(Constants.DATASTREAM.NEW_CONTROL_GROUP.getURI(), new StringAttribute(controlGroup));
			if (dsState != null && !"".equals(dsState)) resAttr.put(Constants.DATASTREAM.NEW_STATE.getURI(), new StringAttribute(dsState));
			...
			LogUtil.statLog(context.getUsername(), Constants.ACTION.ADD_DATASTREAM.getURI().toASCIIString(), pid, dsID);
		}
		catch (Exception e)
		{
