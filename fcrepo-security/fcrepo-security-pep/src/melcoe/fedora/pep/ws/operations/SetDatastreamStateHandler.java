 ...
 *
 */
public class SetDatastreamStateHandler extends AbstractOperationHandler
{
	private static Logger log = Logger.getLogger(SetDatastreamStateHandler.class.getName());

	public SetDatastreamStateHandler() throws PEPException
	{
		super();
	}
	...
	{
		log.debug("SetDatastreamStateHandler/handleRequest!");
		...
		try
		{
			...
			actions.put(Constants.ACTION.ID.getURI(), new StringAttribute(Constants.ACTION.SET_DATASTREAM_STATE.getURI().toASCIIString()));
			...
			LogUtil.statLog(context.getUsername(), Constants.ACTION.SET_DATASTREAM_STATE.getURI().toASCIIString(), pid, dsID);
		}
		catch (Exception e)
		{
