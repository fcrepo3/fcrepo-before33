 ...
 *
 */
public class GetDatastreamHistoryHandler extends AbstractOperationHandler
{
	private static Logger log = Logger.getLogger(GetDatastreamHistoryHandler.class.getName());

	public GetDatastreamHistoryHandler() throws PEPException
	{
		super();
	}
	...
	public RequestCtx handleRequest(MessageContext context) throws OperationHandlerException
	{
		...
		try
		{
			...
			actions.put(Constants.ACTION.ID.getURI(), new StringAttribute(Constants.ACTION.GET_DATASTREAM_HISTORY.getURI().toASCIIString()));
			...
			LogUtil.statLog(context.getUsername(), Constants.ACTION.GET_DATASTREAM_HISTORY.getURI().toASCIIString(), pid, dsID);
}
		catch (Exception e)
		{
