 ...
 * 
 */
public class GetObjectHistory extends AbstractFilter
{
	private static Logger log = Logger.getLogger(GetObjectHistory.class.getName());
	 ...
	 * @throws PEPException
	 */
	public GetObjectHistory() throws PEPException
	{
		super();
	}
	    ...
	    throws IOException, ServletException
	{
		...
		try
		{
			...
			actions.put(Constants.ACTION.ID.getURI(),
			    new StringAttribute(Constants.ACTION.GET_OBJECT_HISTORY.getURI().toASCIIString()));
			actions.put(Constants.ACTION.API.getURI(),
			    new StringAttribute(Constants.ACTION.APIA.getURI().toASCIIString()));

			req = getContextHandler().buildRequest(getSubjects(request),
			    actions,
			    resAttr,
			    getEnvironment(request));

			LogUtil.statLog(request.getRemoteUser(),
			    Constants.ACTION.GET_OBJECT_HISTORY.getURI().toASCIIString(),
			    pid,
			    null);
		}
		catch (Exception e)
