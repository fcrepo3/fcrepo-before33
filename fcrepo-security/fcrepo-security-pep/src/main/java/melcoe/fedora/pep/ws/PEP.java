...
public class PEP extends BasicHandler
{
	...
	public void invoke(MessageContext context) throws AxisFault
	{
		if (log.isDebugEnabled())
			log.debug("AuthHandler executed: " + context.getTargetService() + "/" + context.getOperation().getName()
					+ " [" + ts + "]");
		...
		catch (OperationHandlerException ohe)
		{
			...
			throw AxisFault.makeFault(new PEPException("Error handling operation: " + operation.getName(), ohe));
		}
		...
		catch (PEPException pe)
		{
			...
			throw AxisFault.makeFault(new PEPException("Error evaluating request (operation: " + operation.getName()
					+ ")", pe));
		}
		 ...
		 * obligations are met before providing access.
		 * 
		 * There will need to be an external obligations service that this PEP
		 * can communicate with. Will be working on that...
		 * 
		 * This service will throw an 'Obligations need to be met' exception for
	...
	private void loadHandlers() throws PEPException
	{
		...
		try
		{
			...
			Map<String, OperationHandler> handlerMap = new HashMap<String, OperationHandler>();
			
			NodeList nodes =  doc.getElementsByTagName("handlers-ws");
			for (int x = 0; x < nodes.getLength(); x++)
			{
				String service = nodes.item(x).getAttributes().getNamedItem("service").getNodeValue();
				if (service == null || "".equals(service))
					throw new PEPException("Error in config file: service name missing.");
				
				Map<String, OperationHandler> handlers = serviceHandlers.get(service);
				if (handlers == null)
				{
				...
				for (int y = 0; y < handlerNodes.getLength(); y++)
				{
					...
					{
						String opn = handlerNodes.item(y).getAttributes().getNamedItem("operation").getNodeValue();
						String cls = handlerNodes.item(y).getAttributes().getNamedItem("class").getNodeValue();
						...
						if (handler == null)
						{
							...
							{
								log.error("Could not instantiate handler: " + cls);
						...
						if (log.isDebugEnabled())
							log.debug("handler added to handler map: " + service + "/" + opn + "/" + cls);
					}	
				}
			}
		}
