 ...
 *
 */
public class GetRelationshipsHandler extends AbstractOperationHandler
{
	private static Logger log = Logger.getLogger(GetRelationshipsHandler.class.getName());

	public GetRelationshipsHandler() throws PEPException
	{
		super();
	}
	...
	{
		log.debug("GetRelationshipsHandler/handleRequest!");
		...
		try
		{
			...
			actions.put(Constants.ACTION.ID.getURI(), new StringAttribute(Constants.ACTION.GET_RELATIONSHIPS.getURI().toASCIIString()));
			...
			LogUtil.statLog(context.getUsername(), Constants.ACTION.GET_RELATIONSHIPS.getURI().toASCIIString(), pid, null);
		}
		catch (Exception e)
		{
