 ...
 *
 */
public class ModifyObjectHandler extends AbstractOperationHandler
{
	private static Logger log = Logger.getLogger(ModifyObjectHandler.class.getName());

	public ModifyObjectHandler() throws PEPException
	{
		super();
	}
	...
	public RequestCtx handleRequest(MessageContext context) throws OperationHandlerException
	{
		...
		try
		{
			...
			if (state != null && !"".equals(state)) resAttr.put(Constants.OBJECT.STATE.getURI(), new StringAttribute(state));
			if (ownerId != null && !"".equals(ownerId)) resAttr.put(Constants.OBJECT.OWNER.getURI(), new StringAttribute(state));
			...
			if (state != null && (state.equals("A")))
				...
				actions.put(Constants.ACTION.ID.getURI(), new StringAttribute(Constants.ACTION.MODIFY_OBJECT.getURI().toASCIIString()));
			...
			LogUtil.statLog(context.getUsername(), Constants.ACTION.MODIFY_OBJECT.getURI().toASCIIString(), pid, null);
		}
		catch (Exception e)
		{
