...
public class ObjectsFilter extends AbstractFilter
{
	...
	public RESTFilter getObjectsHandler(HttpServletRequest request) throws ServletException
	{
		...
		if (parts.length == 1)
		{
			...
			else if (request.getParameterMap().containsKey("terms") || request.getParameterMap().containsKey("query"))
				handlerName = Handlers.FINDOBJECTS;
		}
		else if (parts.length == 2 && (isPID(parts[1]) || "new".equals(parts[1])))
		{
		...
		else if (parts.length == 4 && isPID(parts[1]) && "datastreams".equals(parts[2]) &&
		    isDatastream(parts[3]))
		{
			if ("PUT".equals(method) && request.getParameterMap().containsKey("dsState"))
				handlerName = Handlers.SETDATASTREAMSTATE;
			else if ("PUT".equals(method) && request.getParameterMap().containsKey("versionable"))
		...
		else if (parts.length == 5 && isPID(parts[1]) && "datastreams".equals(parts[2]) &&
		    isDatastream(parts[3]) && "content".equals(parts[4]))
		{
			...
			for (int x = 0; x < nodes.getLength(); x++)
			{
				...
				if (n.getNodeType() == Node.ELEMENT_NODE && "handler".equals(n.getNodeName()))
				{
					...
					try
					{
						...
						if (log.isDebugEnabled())
							log.debug("objects handler added to map: " + opn + "/" + cls);
					}
