 ...
 * 
 */
public class ModifyDatastream extends AbstractFilter
{
	private static Logger log = Logger.getLogger(ModifyDatastream.class.getName());
	 ...
	 * @throws PEPException
	 */
	public ModifyDatastream() throws PEPException
	{
		super();
	}
	    ...
	    throws IOException, ServletException
	{
		...
		try
		{
			...
			String action = null;
			if (dsLocation != null && !"".equals(dsLocation))
				action = Constants.ACTION.MODIFY_DATASTREAM_BY_REFERENCE.getURI().toASCIIString();
			else
				action = Constants.ACTION.MODIFY_DATASTREAM_BY_VALUE.getURI().toASCIIString();

			actions.put(Constants.ACTION.ID.getURI(), new StringAttribute(action));
