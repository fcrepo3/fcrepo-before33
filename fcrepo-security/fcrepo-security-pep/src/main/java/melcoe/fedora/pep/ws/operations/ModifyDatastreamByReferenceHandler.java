 ...
 *
 */
public class ModifyDatastreamByReferenceHandler extends AbstractOperationHandler
{
	private static Logger log = Logger.getLogger(ModifyDatastreamByReferenceHandler.class.getName());

	public ModifyDatastreamByReferenceHandler() throws PEPException
	{
		super();
	}
	...
	public RequestCtx handleRequest(MessageContext context) throws OperationHandlerException
	{
		...
		try
		{
			...
			if (mimeType != null && !"".equals(mimeType)) resAttr.put(Constants.DATASTREAM.NEW_MIME_TYPE.getURI(), new StringAttribute(mimeType));
			if (formatURI != null && !"".equals(formatURI)) resAttr.put(Constants.DATASTREAM.NEW_FORMAT_URI.getURI(), new AnyURIAttribute(new URI(formatURI)));
			if (dsLocation != null && !"".equals(dsLocation)) resAttr.put(Constants.DATASTREAM.NEW_LOCATION.getURI(), new AnyURIAttribute(new URI(dsLocation)));
			if (checksumType != null && !"".equals(checksumType)) resAttr.put(Constants.DATASTREAM.NEW_CHECKSUM_TYPE.getURI(), new StringAttribute(checksumType));
			if (checksum != null && !"".equals(checksum)) resAttr.put(Constants.DATASTREAM.NEW_CHECKSUM.getURI(), new StringAttribute(checksum));

			actions.put(Constants.ACTION.ID.getURI(), new StringAttribute(Constants.ACTION.MODIFY_DATASTREAM_BY_REFERENCE.getURI().toASCIIString()));
			...
			LogUtil.statLog(context.getUsername(), Constants.ACTION.MODIFY_DATASTREAM_BY_REFERENCE.getURI().toASCIIString(), pid, dsID);
		}
		catch (Exception e)
		{
