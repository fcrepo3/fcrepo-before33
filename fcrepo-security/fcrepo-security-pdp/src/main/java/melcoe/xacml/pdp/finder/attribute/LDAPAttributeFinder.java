...
public class LDAPAttributeFinder extends AttributeFinderModule
{
	private static final Logger log = Logger.getLogger(LDAPAttributeFinder.class);
	...
	public LDAPAttributeFinder()
	{
		try
		{
			...
			options = AttributeFinderConfigUtil.getOptionMap(this.getClass().getName());
	 ...
	 * 
	 * @param attributeType the datatype of the attributes to find, which must
	 *        be time, date, or dateTime for this module to resolve a value
	 * @param attributeId the identifier of the attributes to find, which must
	 *        be one of the three ENVIRONMENT_* fields for this module to
	 *        resolve a value
	 ...
	 *        ENVIRONMENT_TARGET for this module to resolve a value
	 * 
			...
			while (ne.hasMore())
			{
				...
				while (neas.hasMoreElements())
				{
					...
					while (nea.hasMoreElements())
					{
						...
						try
						{
							attributeValue = attributeFactory.createValue(type, value);
						}
						catch (Exception e)
						{
						...
						if (log.isDebugEnabled())
							log.debug("AttributeValue found: [" + type.toASCIIString() + "] " + value);
					}
				}
			}
		}
