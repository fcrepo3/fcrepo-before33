...
public class LDAPAttributeFinder extends AttributeFinderModule {
	...
	public LDAPAttributeFinder() {
		try {
			...
			options = AttributeFinderConfigUtil.getOptionMap(this.getClass()
					.getName());
	...
	public EvaluationResult findAttribute(URI attributeType, URI attributeId,
		...
		try {
			...
			URI category = new URI(
					"urn:oasis:names:tc:xacml:1.0:subject-category:access-subject");

			EvaluationResult userER = context.getSubjectAttribute(userType,
					userId, category);
			if (userER == null)
	...
	private EvaluationResult getEvaluationResult(String user, String attribute,
			URI type) {
			...
			while (ne.hasMore()) {
				...
				while (neas.hasMoreElements()) {
					...
					while (nea.hasMoreElements()) {
						...
						try {
							attributeValue = attributeFactory.createValue(type,
									value);
						} catch (Exception e) {
	...
	public static void main(String[] args) throws Exception {
		...
		EvaluationResult result = finder.getEvaluationResult("nishen",
				"eduPersonEntitlement", type);
