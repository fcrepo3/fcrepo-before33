...
{
	private static final Logger log = Logger.getLogger(AttributeFinderConfigUtil.class.getName());

	public static Map<Integer, Set<String>> getAttributeFinderConfig(String className) throws AttributeFinderException
	{
		...
		try
		{
			...
			log.info("Loading attribute finder config file: " + f.getAbsolutePath());
			...
			for (int x = 0; x < nodes.getLength(); x++)
			{
				...
				if (className.equals(name))
				{
					...
					for (int y = 0; y < attributes.getLength(); y++)
					{
						...
						if (n.getNodeType() == Node.ELEMENT_NODE && "attribute".equals(n.getNodeName()))
						{
							String designator = n.getAttributes().getNamedItem("designator").getNodeValue();
							...
							Integer designatorValue = new Integer(designatorTable.indexOf(designator));
							String attribute = n.getAttributes().getNamedItem("name").getNodeValue();

							Set<String> attrs = attributeSet.get(designatorValue);
							if (attrs == null)
							{
		...
		{
			log.fatal("Could not initialise AttributeFinder: [" + className + "] " + e.getMessage(), e);
			throw new AttributeFinderException("Could not initialise AttributeFinder: [" + className + "] "
					+ e.getMessage(), e);
		}
	...
	public static Map<String, String> getResolverConfig(String className) throws AttributeFinderException
	{
		...
		try
		{
			...
			{
				Node n = attributes.item(y);
				if (n.getNodeType() == Node.ELEMENT_NODE && "resolver".equals(n.getNodeName()))
				{
					config.put("url", n.getAttributes().getNamedItem("url").getNodeValue());
					config.put("username", n.getAttributes().getNamedItem("username").getNodeValue());
					config.put("password", n.getAttributes().getNamedItem("password").getNodeValue());
				}
			}
		}
		catch (Exception e)
	...
	public static Map<String, String> getOptionMap(String className) throws AttributeFinderException
	{
		...
		try
		{
			...
			NodeList attributes = attributeFinder.getElementsByTagName("option");
			for (int y = 0; y < attributes.getLength(); y++)
			{
				Node n = attributes.item(y);
	...
	private static Element getAttributeFinder(String className) throws Exception
	{
		String home = System.getenv("MELCOEPDP_HOME");
		if (home == null || "".equals(home))
			throw new MelcoePDPException("Environment home (MELCOEPDP_HOME) is not set.");
		...
		throw new AttributeFinderException("AttributeFinder not found: " + className);
	}
}
