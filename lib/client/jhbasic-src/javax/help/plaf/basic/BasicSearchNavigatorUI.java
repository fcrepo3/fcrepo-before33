 ...
 */
/*
 * @(#) BasicSearchNavigatorUI.java 1.76 - last change made 04/11/01
 */
             ...
             ComponentListener
{
    ...
    public BasicSearchNavigatorUI(JHelpSearchNavigator b) {
	    ...
	    setIcon(BasicHelpUI.getIcon(BasicSearchNavigatorUI.class, "images/search.gif"));
    }
	...
	public synchronized void actionPerformed(ActionEvent e) {
	    HelpModel helpmodel = searchnav.getModel();
            debug("action:"+helpmodel.getHelpSet().toString());
	    try {
		if (paramCursor == null) {
	...
	*/
    ...
    /**
     * Reloads data from new model, creates new search engine to search in new model
     **/ 
	...
	debug("reloadData using new model");
        ...
        SearchView view = (SearchView)newHelpSet.getNavigatorView("Search");
	...
	debug("  model: "+searchnav.getModel());
	...
	if (item != null) {
		...
		for (Enumeration enum = item.getSearchHits();
		    ...
		    h[i] = new DefaultHighlight(info.getBegin(), 
						   info.getEnd());
		    i++;
		}
