...
//            XmlChars, EntityDecl, ContentModel
...
public final class Parser {
    ...
    private Parser$NameCache nameCache;
    ...
    static final Parser$Catalog messages = new Parser$Catalog();
    ...
    private Parser$FastStack stack;
    private Parser$PIQueue piQueue;
    ...
    private void init() {
        ...
        nameTmp = new char[20];
        nameCache = new Parser$NameCache();
        if(namespace)
            if(ns == null)
    ...
    private String maybeGetName() throws IOException, ParseException {
        Parser$NameCacheEntry entry = maybeGetNameCacheEntry();
        return entry != null ? entry.name : null;
    }

    private Parser$NameCacheEntry maybeGetNameCacheEntry() throws IOException, ParseException {
        char c = getc();
    ...
    private Parser$NameCacheEntry nameCharString(char c) throws IOException, ParseException {
        int i = 1;
...
label0:
    ...
    public Parser(File file, boolean coalescing, boolean namespaceAware) throws IOException {
        ...
        stack = new Parser$FastStack(this, 100);
        piQueue = new Parser$PIQueue(this, 10);
        haveAttributes = false;
        hasContent = true;
        InputStream in = new BufferedInputStream(new FileInputStream(file));
        InputSource is = new InputSource(in);
        is.setSystemId(file.toURL().toString());
        locator = new Parser$DocLocator(this);
        input = is;
    ...
    private Parser(InputSource input, boolean coalescing, boolean namespaceAware) {
        curName = null;
        curValue = null;
        curURI = null;
        parts = new String[3];
    ...
    private ElementDecl getElement() throws IOException, ParseException {
        Parser$NameCacheEntry name = maybeGetNameCacheEntry();
        if(name == null)
            return null;
    ...
    private void intRefEpilogue(Parser$StackElement elt) throws IOException, ParseException {
    ...
    private void extRefEpilogue(Parser$StackElement elt) throws IOException, ParseException {
    ...
    private int parseContent() throws IOException, ParseException {
        ...
        do {
            ...
            Parser$StackElement se = stack.pop();
            elt = se.elt;
            switch(se.curState) {
            case 256: 
                if(attr == null)
    ...
    private Parser$StackElement newStackElement(int origState, int curState, ElementDecl elt, EntityDecl entity, InputEntity in) {
        return new Parser$StackElement(this, origState, curState, elt, entity, in);
    }

    private void freeStackElement(Parser$StackElement parser$stackelement) {
    }
    ...
    static InputEntity access$000(Parser x0) {
        return x0.in;
    }
