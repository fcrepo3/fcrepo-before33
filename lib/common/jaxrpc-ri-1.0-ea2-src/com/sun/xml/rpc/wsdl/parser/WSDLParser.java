...
//            SchemaParser, ExtensionHandler, Util
...
public class WSDLParser {
    ...
    protected Definitions parseDefinitions(ParserContext context, InputSource source, String expectedTargetNamespaceURI) {
        ...
        processImports(context, definitions);
        return definitions;
    }

    protected void processImports(ParserContext context, Definitions definitions) {
        for(Iterator iter = definitions.imports(); iter.hasNext();) {
            ...
            if(location != null && !context.getDocument().isImportedDocument(location)) {
                ...
                try {
                    context.getDocument().addImportedEntity(parseDefinitions(context, new InputSource(adjustedLocation), i.getNamespace()));
                }
    ...
    protected Definitions parseDefinitionsNoImport(ParserContext context, InputSource source, String expectedTargetNamespaceURI) {
        try {
            ...
            builder.setErrorHandler(new WSDLParser$1(this));
    ...
    protected Definitions parseDefinitionsNoImport(ParserContext context, Element e, String expectedTargetNamespaceURI) {
        ...
        _targetNamespaceURI = XmlUtil.getAttributeOrNull(e, "targetNamespace");
    ...
    protected Operation parsePortTypeOperation(ParserContext context, Element e) {
        ...
        for(Iterator iter = XmlUtil.getAllChildren(e); iter.hasNext();) {
            ...
            if(XmlUtil.matchesTagNS(e2, WSDLConstants.QNAME_INPUT)) {
                if(gotInput)
                    Util.fail("parsing.tooManyElements", new Object[] {
    ...
    protected boolean handleExtension(ParserContext context, Extensible entity, Element e) {
        ExtensionHandler h = (ExtensionHandler)_extensionHandlers.get(e.getNamespaceURI());
        if(h == null) {
            context.fireIgnoringExtension(new QName(e.getNamespaceURI(), e.getLocalName()), ((com.sun.xml.rpc.wsdl.framework.Entity)entity).getElementName());
            return false;
        } else {
